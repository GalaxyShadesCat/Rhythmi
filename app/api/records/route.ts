...
const getMongoClient = async () => {
  ...
  console.log(`Connected to MongoDB database: ${DATABASE_NAME} via MongoClient`);
  return client;
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`Looking for records in ${DATABASE_NAME}.records for user: ${user_id}`);
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`Saving record to ${DATABASE_NAME}.records for user: ${recordData.user_id}`);
    console.log('Record data sample:', JSON.stringify({
    ...
    console.log(`Record saved successfully with ID: ${result.insertedId} to ${DATABASE_NAME}.records`);
