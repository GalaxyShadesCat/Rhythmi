// hooks/predictRecovery.ts
import { ECGDataPoint } from "@/hooks/useHeartRateSensor";
import { BaseMetrics } from "./calculateBaseMetrics";
...
export default function predictRecovery(
  ...
  while (currentWindowStart + windowSize <= ecgData[ecgData.length - 1].timestamp) {
    const windowEnd = currentWindowStart + windowSize;
    const windowData = ecgData.filter(
      point => point.timestamp >= currentWindowStart && point.timestamp < windowEnd
    );

    if (windowData.length > 0) {
      const windowMetrics = calculateWindowMetrics(windowData);
      const isSimilar = compareMetrics(windowMetrics, baseMetrics, similarityThreshold);
...
function calculateWindowMetrics(windowData: ECGDataPoint[]): { mean: number; variance: number } {
...
function compareMetrics(
  ...
  const varianceDiff = Math.abs(current.variance - baseline.variance) / baseline.variance;
