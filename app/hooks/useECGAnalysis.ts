...
export function useECGAnalysis(ecgData: ECGDataPoint[]) {
  ...
  const findPeaks = useCallback((data: ECGDataPoint[]): number[] => {
    ...
    console.log(`Signal stats - Mean: ${mean.toFixed(2)}, StdDev: ${stdDev.toFixed(2)}, SNR: ${snr.toFixed(2)}`);
    console.log(`Using threshold factor: ${thresholdFactor}, Threshold: ${adaptiveThreshold.toFixed(2)}`);
    ...
    const potentialPeaks: {index: number, value: number, timestamp: number}[] = [];
    for (let i = 2; i < data.length - 2; i++) {
      // Looking at a 5-point window to find local maxima
      const isPeak = data[i].value > data[i-2].value && 
  ...
  const calculateHeartRate = useCallback((rPeaks: number[]): number => {
    ...
    console.log(`Calculated heart rate: ${heartRate} BPM from ${rrIntervals.length} valid RR intervals`);
    return heartRate;
  }, []);
  ...
  const calculateHRV = useCallback((rPeaks: number[]): number => {
    ...
    for (let i = 1; i < recentIntervals.length; i++) {
      successiveDiffs.push(Math.abs(recentIntervals[i] - recentIntervals[i - 1]));
    }
    ...
    const meanSquaredDiff = squaredDiffs.reduce((sum, diff) => sum + diff, 0) / squaredDiffs.length;
    const rmssd = Math.sqrt(meanSquaredDiff);
    
    console.log(`Calculated HRV (RMSSD): ${rmssd.toFixed(2)} ms from ${successiveDiffs.length} intervals`);
    return rmssd;
  }, []);

  // Track heart rate changes for recovery monitoring
  const trackHeartRateChanges = useCallback((currentHR: number, timestamp: number) => {
    ...
    setMetrics(prevMetrics => {
      ...
      // If baseline is set
      if (newMetrics.baselineTime !== null && newMetrics.baselineHeartRate > 0) {
        // 1. Check for exercise start (if not already started)
        if (newMetrics.exerciseStartTime === null) {
          const hrIncrease = ((currentHR - newMetrics.baselineHeartRate) / newMetrics.baselineHeartRate) * 100;
          if (hrIncrease >= HR_RISE_THRESHOLD) {
            console.log(`Exercise started: HR increased by ${hrIncrease.toFixed(1)}% above baseline`);
        ...
        if (newMetrics.exerciseStartTime !== null) {
          ...
          if (newMetrics.recoveryStartTime === null && 
              newMetrics.peakHeartRate > 0 && 
              newMetrics.peakHeartRateTime !== null) {
            // Only check for recovery if we've been over the peak for a bit
            if (now - newMetrics.peakHeartRateTime > 5000) { // 5 seconds after peak
              const hrDecrease = ((newMetrics.peakHeartRate - currentHR) / newMetrics.peakHeartRate) * 100;
              if (hrDecrease >= HR_RECOVERY_THRESHOLD) {
                console.log(`Recovery started: HR decreased by ${hrDecrease.toFixed(1)}% from peak`);
          ...
          if (newMetrics.recoveryStartTime !== null && newMetrics.recoveryTime === null) {
            const diffFromBaseline = Math.abs((currentHR - newMetrics.baselineHeartRate) / newMetrics.baselineHeartRate) * 100;
            if (diffFromBaseline <= HR_BASELINE_THRESHOLD) {
              const recoveryTimeMs = now - (newMetrics.recoveryStartTime || now);
              console.log(`Recovery completed in ${(recoveryTimeMs / 1000).toFixed(1)} seconds`);
  ...
  const setBaseline = useCallback((hr: number, timestamp: number) => {
    ...
    console.log(`Set baseline heart rate: ${hr} BPM at ${new Date(timestamp).toLocaleTimeString()}`);
  }, []);
  ...
  useEffect(() => {
    ...
    const latestTimestamp = windowData[windowData.length - 1]?.timestamp || Date.now();
  ...
  }, [ecgData, findPeaks, calculateHeartRate, calculateHRV, trackHeartRateChanges]);
  
  // Return the metrics, hrHistory and setBaseline function
  return { 
    metrics, 
    hrHistory, // Include heart rate history in the return value
    setBaseline: (timestamp = Date.now()) => setBaseline(metrics.heartRate, timestamp)
  };
}